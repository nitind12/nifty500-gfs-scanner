...
TF_CHOICE = os.environ.get("TF_CHOICE", "8")
...
def main():
    ...
    if not results:
        ...
        empty_path = os.path.join(OUTPUT_DIR, f"engulfing_scan_weekly_{datetime.now().strftime('%Y%m%d')}.csv")
    ...
    out_path = os.path.join(script_dir, f"engulfing_scan_weekly_{datetime.now().strftime('%Y%m%d')}.csv")
