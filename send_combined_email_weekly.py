"""
Sends ONE weekly email with the latest CSV from each scanner found in OUTPUT_DIR.
...
def main():
    ...
    msg["Subject"] = f"Weekly Scanner Results - {dt.date.today().isoformat()} ({len(csv_files)} scanners)"

    body_lines = [
        "Weekly scan run complete. Latest result from each scanner is attached:\n",
    ]
    for f in csv_files:
    ...
    try:
        ...
        server.quit()
        print(f"Combined weekly email sent to {RECIPIENT_EMAIL} with {len(csv_files)} latest attachment(s).")
