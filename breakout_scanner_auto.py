...
INTERVAL = "1d"
TF_LABEL = "Daily"
LOOKBACK_BARS = 52 * 5   # 52 weeks * 5 trading days/week = 260
LOOKBACK_LABEL = "52 Weeks"
...
def analyze_stock(ticker: str) -> dict | None:
    try:
        years = max(2, (LOOKBACK_BARS // 252) + 1)
        period = f"{years}y"
        df = yf.download(ticker, period=period, interval=INTERVAL, progress=False, auto_adjust=True)
        if df.empty or len(df) < max(60, LOOKBACK_BARS):
            return None
...
def main():
    ...
    dated_path = os.path.join(OUTPUT_DIR, f"breakout_scan_{date_str}.csv")
    latest_path = os.path.join(OUTPUT_DIR, "breakout_scan_latest.csv")
